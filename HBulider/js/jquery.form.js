    ...
    $.fn._submit= function(options){
    	var defaultOptions = {
    		...
    		callback: function(data){
    			...
    			}else{//若ResponseVO为空，则去处理“规则”里的数据
    				
    				//若description不为空且无传入success函数，则提示description的内容
    				if(!isEmpty(data) && !isEmpty(data.description) && isEmpty(defaultOptions.success)){
						$.alert("提示", data.description, "remind");
    				}
    				
    				//下拉框、单选框、复选框在数据字典的取值
    				if(!isEmpty(data["rule"]) && !isEmpty(data["rule"]["cdi"])){
    					//下拉框在数据字典的取值
    					setCombBox(data["rule"]["cdi"]);
    					//单选框、复选框在数据字典的取值
    					setRadioCheckBox(data["rule"]["cdi"]);
    				}
    				
    				//显隐控制的处理
    				if(!isEmpty(data["rule"]) && !isEmpty(data["rule"]["visit"])){
    					setVisitMode(data["rule"]["visit"]);
    				}
    				
    				//数据赋值
    				if(!isEmpty(data["rule"]) && !isEmpty(data["rule"]["data"])){
    					setFieldValue(data["rule"]["data"]);
    				}
    				
    				//消息提示
    				if(!isEmpty(data["rule"]) && !isEmpty(data["rule"]["message"])){
    					setMessage(data["rule"]["message"]);
    				}
    				
    				//后台js脚本
    				if(!isEmpty(data["rule"]) && !isEmpty(data["rule"]["script"])){
    					handleScript(data["rule"]["script"]);
    				}
    			}
    		}
    	};
    	...
    	if(useAjax){
    		...
    		dataValue["data"]=strEnc(JSON.stringify(dataValue),_des_key);
...
//	    	}
    ...
    $.fn.getFormData= function(){
    	...
    	var _objRegexp = new RegExp("^ueditor_textarea");
		var data = {};
    	$(this).find(':text,:hidden,:password,textarea,:checkbox:checked,:radio:checked,select').not(':button,:submit,:reset').each(function(i){
		    if($(this).attr('name') != null && $(this).attr('name') != "" && contain(_ignoreFieldName,$(this).attr('name'))<0){
		        ...
		        if(_ignoreEmpty==false || _obj.getValue()!=''){
		        	var objVale = _obj.getValue();
		        	if(null != objVale){
						if (_objRegexp.test(_obj.attr("id"))) {
							objVale = strEnc(objVale, _des_key);
						} else {
							objVale = objVale.replace(/\'/g, "‘").replace(/\"/g, "“");
						}
		        	}
		        	if(contain(jsonFiled.join(","),_obj.attr("name"))>=0){
		        		...
		        		_setJsonData(data,_obj.attr("name"),objVale);
		        	}else{
		        		_setJsonData(data,_obj.attr("name"),objVale);
		        	}
    			}
    ...
    $.fn.leaveConfirm = function(){
    	...
    	$(this).find(":input").not("button").on("change",function(){
    		...
    		console.log(JSON.stringify(formDataArray[_key]));
